...
- [~] **Phase 3 — Customer portal.** ✅ My Orders + order detail + animated dog tracker + proof approve/request-changes; My Designs, Account, Help. (Remaining: dashboard action-items, reorder, account editing, saved-artwork reuse.)
- [~] **Phase 4 — Admin core.** ✅ Orders list (`roughadmin.png`) + order detail (`roughadmin2.png`): line items, editable pricing, official mockup→proof, internal/customer notes, status control (drives tracker), payment status, activity log — full proof loop browser-QA'd (DH-2026-0001 designed→ordered→proofed→approved→in-production). (Remaining: Proofs queue, Production board, Quotes, Customers CRM, Reports, Settings, admin Dashboard.)
