...
- [x] **Phase 3 — Customer portal.** ✅ Dashboard (action-items + active-order trackers), My Orders + order detail + animated dog tracker + proof approve/request-changes, reorder, My Designs, Account, Help. (Remaining: account editing form, saved-artwork reuse in designer.)
- [x] **Phase 4 — Admin core.** ✅ Dashboard (queues/alerts/activity), Orders list + detail (status control→tracker, mockup→proof, notes, editable pricing, payment, activity), Proofs & Artwork queue, Production kanban (stage moves QA'd), Quotes inbox, Customers CRM (+profile), Reports (+CSV export), Settings (decoration methods/staff/email templates). Full §8.1 loop browser-QA'd.
- [~] **Phase 5 — Org & cross-cutting.** ✅ Resend transactional notifications wired to status changes (templates + prefs); reports CSV. (Remaining: Organizations / team accounts + shared artwork/brand kit in designer.)
