...
2. **P0 + P1 brand pass is done** (storefront/designer/portal warmed + abundant catalog + alignment fixed, all browser-verified). **Remaining P2 / PRD work**: Organizations / team accounts (§6 — entity exists; needs member-management UI + shared artwork/brand kit, selectable in the designer); saved-artwork-on-file reuse in the designer + "My Designs" (§5.5); multi-view mockup export (designer still exports the active view only); text curve/arch + clip-art library; admin category CRUD; bulk-tier quote pricing; art-issue flags on proofs.
