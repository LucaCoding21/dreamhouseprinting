...
2. **URGENT — the `SUPABASE_ACCESS_TOKEN` in `.env.local` is dead (401 from the Management API), so migrations `0014_column_guards.sql` AND `0015_etransfer_payments.sql` are NOT applied** (verified via `_app_migrations`). Mint a new personal access token (supabase.com/dashboard/account/tokens), replace `SUPABASE_ACCESS_TOKEN`, run `node --env-file=.env.local scripts/db-push.mjs`. Until then, ANY payment marking (Stripe webhook included) errors at runtime: `markOrderPaid` now writes the not-yet-existing `payment_method` column. Then set the e-transfer email in Admin → Settings → Payments and browser-QA the flow (customer report → dashboard queue → confirm/not-received, plus a Stripe card payment for regression).
